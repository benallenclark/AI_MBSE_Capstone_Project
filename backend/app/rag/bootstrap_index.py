...
model_dir = jsonl_path.parent.parent           # .../<model_id>/evidence/evidence.jsonl -> .../<model_id>
...
(doc_id, model_id, vendor, version, mml, probe_id, doc_type, subject_type, subject_id,
 title, ctx_hdr, body_text, json_metadata, ts_ms)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
"""
...
def iter_rows():
    ...
    # Beware empty `model_id`/`probe_id`—you can end up with ambiguous IDs.
                j.get("doc_id") or f"{md.get('model_id','')}/{j.get('probe_id','')}/{n}",
                md.get("model_id"),
                md.get("vendor"),
                md.get("version"),
                j.get("mml"),
