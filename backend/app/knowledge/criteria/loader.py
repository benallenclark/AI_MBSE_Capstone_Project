...
# - Only scan immediate package tree under app.knowledge.criteria.
...
def discover(
    groups: Iterable[str] | None = None, strict: bool = True
) -> list[tuple[str, str, Predicate]]:
    wanted = set(groups) if groups else None
    results: list[tuple[str, str, Predicate]] = []

    print(
        f"[loader] start discovery base={_BASE} wanted={sorted(wanted) if wanted else 'ALL'}",
        flush=True,
    )
    ...
    for _, modname, ispkg in pkgutil.walk_packages(
        [str(_BASE)], prefix=f"{__package__}."
    ):
        if ispkg:
            # packages are just containers; skip
            continue

        # Expected layout: app.knowledge.criteria.{mml_N}.predicate_*
        # Skip any modules that don't match the depth or naming convention.
        parts = modname.split(".")
        if len(parts) < 4:
            # Expect: app.knowledge.criteria.mml_X.predicate_*
            continue
    ...
    print(
        f"[loader] discovery complete: {len(results)} loaded -> {summary}", flush=True
    )
    if import_errors and strict:
        raise RuntimeError(
            f"Predicate import failures: {[(m, type(e).__name__) for m, e in import_errors]}"
        )
    return results
