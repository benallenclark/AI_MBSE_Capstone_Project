...
class PredicateCrashed(Exception):
    def __init__(self, group, pid, err): super().__init__(f"{group}:{pid} crashed: {type(err).__name__}: {err}")
...
def run_predicates(
    ...
    print(f"[runner] begin model_id={model_id} vendor={vendor} version={version} groups={groups or 'ALL'}", flush=True)
    ...
    for idx, (group, pid, fn) in enumerate(loaded, start=1):
        ...
        try:
            ...
            print(f"[runner] ERROR {group}:{pid} → {type(ex).__name__}: {ex}", flush=True)
            traceback.print_exc()
            ...
            if err:
                print(f"[runner] DONE {group}:{pid} status={status} dur_ms={dur_str}{slow} ERROR={type(err).__name__}: {err}", flush=True)
            else:
                print(f"[runner] DONE {group}:{pid} status={status} passed={ok} dur_ms={dur_str}{slow}", flush=True)
        
        # Stable predicate ID "mml_N:predicate_name" for evidence and UI mapping.
        norm_id = f"{group}:{pid}"
        if err is None:
            evidence.append(EvidenceItem(predicate=norm_id, passed=ok, details=details_dict))
            ...
            evidence.append(EvidenceItem(predicate=norm_id, passed=False, details={}, error=str(err)))
    ...
    for lvl in sorted(expected_by_level.keys()):
        ...
        if not all_passed:
            break
        maturity_level = lvl
    print(f"[runner] complete model_id={model_id} maturity_level={maturity_level}/{len(loaded)}", flush=True)
    ...
    for lvl in sorted(expected_by_level.keys()):
        ...
        for pid in sorted(want):
            ...
            summary = None
            # Normalize an optional universal summary {ok,total,ratio,fail} for simple UI bars.
            if isinstance(measure, dict) and ("ok" in measure or "total" in measure or "ratio" in measure):
                ok = int(measure.get("ok", 0) or 0)
                total = int(measure.get("total", 0) or 0)
                ratio = float(measure.get("ratio", (ok/total if total else 0.0)))
                summary = {"ok": ok, "total": total, "ratio": ratio, "fail": max(0, total - ok)}
            ...
            # assert not any(k in entry_clean for k in ("probe_id","title","ts_ms")), entry_clean
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Run maturity predicates and emit evidence.jsonl")
    ap.add_argument("--model-dir", type=Path, required=True, help="Path to model dir (…/data/models/<id>)")
    ap.add_argument("--vendor", type=str, default="", help="Vendor (e.g., sparx, cameo)")
    ap.add_argument("--version", type=str, default="", help="Vendor version (e.g., 17.1)")
    args = ap.parse_args()
    ...
    fp_src = [
        {"id": e.predicate, "passed": bool(e.passed), "keys": sorted(list((e.details or {}).keys()))}
    ...
    fingerprint = hashlib.sha256(json.dumps(fp_src, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
    ...
    paths.summary_json(model_id).write_text(json.dumps(summary, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    print(f"[runner] exit maturity_level={level} evidence_items={len(evidence)} summary.json=written", flush=True)
