# ------------------------------------------------------------
# Module: app/interface/api/v1/analyze.py
...
from app.infra.core.jobs_db import create_job
from app.infra.utils.hashing import compute_sha256
from app.interface.api.v1.serializers.jobs import to_job_payload as _payload
from app.interface.bridge.analysis import (
    post_ingest_best_effort,
    run_pipeline_job,
    run_sync_predicates,
)
from app.interface.bridge.jobs import get_or_synthesize_job_row, persist_model_xml
...
async def analyze_upload(
    ...
    # Always create a fresh job (caching disabled)
    ...
    response.headers["Location"] = f"/v1/jobs/{job_id}"
    log.info(
        "upload queued (no-cache): sha=%s vendor=%s version=%s job_id=%s model_id=%s",
        sha,
        vendor.value,
        version,
        job_id,
