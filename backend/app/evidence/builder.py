...
# Millisecond epoch for ts_ms fields; prefer a single clock source across docs.
def _now_ms() -> int:
...
def _to_mapping(obj: Any) -> Dict[str, Any]:
    ...
    if _is_dataclass_instance(obj):
        ...
        return asdict(obj)
    out: Dict[str, Any] = {}
    for k in ("probe_id","mml","counts","facts","source_tables","category","rule","severity","measure","refs"):
        if hasattr(obj, k):
            out[k] = getattr(obj, k)
    return out
...
def _fact_to_mapping(obj: Any) -> Dict[str, Any]:
    ...
    if _is_dataclass_instance(obj):
        ...
        return asdict(obj)
    out: Dict[str, Any] = {}
    for k in ("subject_type","subject_id","subject_name","has_issue","child_count","tags","meta","refs","quotes"):
        if hasattr(obj, k):
            out[k] = getattr(obj, k)
    return out
...
class EvidenceBuilder:
    ...
    def emit(self, ctx: Dict[str, Any], out: Any) -> List[Dict[str, Any]]:
        ...
        # Optional normalized measure {ok,total,ratio} for quick UI bars.
        if "measure" in outd:
        ...
        for fobj in (outd.get("facts") or []):
            ...
            ent_doc: Dict[str, Any] = {
                ...
                "title": self._default_title(pid, subject_type, subject_name, tags, has_issue, child_count),
                "body": self._default_body(pid, subject_type, subject_name, has_issue, child_count),
    ...
    def _default_title(self, pid: str, subject_type: str, subject_name: str, tags: List[str], has_issue: bool, child_count):
        frag: List[str] = []
    ...
    def _default_body(self, pid: str, subject_type: str, subject_name: str, has_issue: bool, child_count):
        claim = ""
        if pid.endswith(".block_has_port") and subject_type == "block":
            if child_count is not None:
                claim = (f"Finding: Block '{subject_name}' has {child_count} port(s)."
                         if not has_issue else
                         f"Finding: Block '{subject_name}' has 0 ports.")
        if not claim:
